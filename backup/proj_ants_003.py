...
class Ants():
    ...
    def update(self, data):
        ...
        # reset vision
        self.gen_vision_field() #self.vision = None
    ...
    def gen_vision_field(self):
        for r in self.rows:
            for c in self.cols:
                self.loc[r][c].visible = False
        self.vision = set()
        for ant in self.my_ants():
            a_row, a_col = ant
            for v in self.vision_offsets:
                L = self.loc[a_row + v.r][a_col + v.c]
                L.visible = True
                self.vision.add(L)
    ...
    ##def visible(self, loc):
        ##"""Determine if loc is visible to the given player. 
        
        ##Also calculates entire field of vision.
                    ...
                    ###for d_col in range(-mx,mx+1):
                        ...
                        ###if d <= self.viewradius2:
                            ###self.vision_offsets_2.append((
                                ...
                                #### (since negative indices wrap around
                                #### in python)
                                ###(d_row % self.rows) - self.rows,
                                ###(d_col % self.cols) - self.cols
                            ###))
                #####log.debug("offsets: %s", self.vision_offsets_2)
