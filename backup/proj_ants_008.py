...
class Ants():
    ...
    def setup(self, data):
        ...
        for r in xrange(self.rows//2):
            for c in xrange(self.cols):
                self.loc[r][c].gen_ranges()
        range_time = time.time() - time_before_range
        halfcells = self.rows//2*self.cols
        cell_time = range_time / halfcells
        log.debug('Used on gen_ranges p1 = {0} seconds'.format(range_time))
        log.debug('Per cell: {0} milliseconds'.format(cell_time*1000))
        time_before_range = time.time()
        # second half
        for r in xrange(self.rows//2, self.rows):
            for c in xrange(self.cols):
                self.loc[r][c].gen_ranges()
        range_time = time.time() - time_before_range
        log.debug('Used on gen_ranges p2 = {0} seconds'.format(range_time))       
