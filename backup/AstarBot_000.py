    ...
    def move_astar(self, ants, start, dest):
        ...
        for direction in ('n','w','s','e'):
            ...
            if adj.unoccupied and adj not in self.orders:
                if adj == dest:
                    ...
                    success = do_move_direction(direction)
                    assert success
                    return success
                dist[adj] = 1
        ...
        while heap:
            ...
            for next_dir in ('n','w','s','e'):
                ...
                if new_loc == dest:
                    # goal reached, assume no better path
                    success = do_move_direction(direction)
                    assert success
                    return success
                if new_loc not in done and new_loc not in heap and new_loc not in blocked:
                    if new_loc.terrain is LAND:
                    ...
                    elif new_loc.terrain is UNKNOWN:
                        # not perfect but assume best
                        # goal reached, assume no better path
                        success = do_move_direction(direction)
                        assert success
                        return success
