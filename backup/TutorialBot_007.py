...
class MyBot:
    ...
    def do_setup(self, ants):
        ...
        self.prev_explore_time = 0.01*ants.rows*ants.cols # milliseconds
    ...
    def do_turn(self, ants):
        ...
        log.debug("%s of %s ms left after unknown update", ants.time_remaining(), ants.turntime)
        for ant_loc in ants.my_ants():
            if ant_loc not in orders.values():
                ...
                log.debug("%s ms used in ant exploration", self.prev_explore_time)
        ...
        log.debug("%s of %s ms left after bot turn", ants.time_remaining(), ants.turntime)
