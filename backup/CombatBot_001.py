...
class MyBot(object):
    ...
    def diffuse_food(self, ants):
        ...
        diff_all = np.concatenate(([1-diff_self],
                                [diff_n],
                                [diff_s],
                                [diff_w],
                                [diff_e]))
        ...
        for diffusion in xrange(MyBot.U_ITERATIONS):
            ...
            # build new scents
            scent_all = np.concatenate((
                [scent_field],
                [np.concatenate((scent_field[-1:], scent_field[:-1]), axis=0)],
                [np.concatenate((scent_field[1:], scent_field[:1]), axis=0)],
                [np.concatenate((scent_field[:,-1:], scent_field[:,:-1]), axis=1)],
                [np.concatenate((scent_field[:,1:], scent_field[:,:1]), axis=1)]
                ))
            parts = diff_all * scent_all
            scent_field = parts[0]+parts[1]+parts[2]+parts[3]+parts[4]
