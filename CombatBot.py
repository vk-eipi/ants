...
CombatBot
v4.3.2 (017)
- fixed bug: a_scent inconsistency due to adding to empty value
v4.3.1 (016)
- fixed bug: realized "stay in place" move will never be unoccupied_next
- fixed bug: changed self to ant_loc in movement code
- fixed bug when "stay in place" is NEUTRAL and best move
v4.3 (015)
- Memetix-style combat safety
- u_scent does not repel if soldiers or enemies
- clamp nearby enemy a_scents
- add enemies to exploration border
- decrease diffusion iterations to prevent timeout
- sort ants before movement 
...
class MyBot(object):
    def __init__(self):
        ...
        MyBot.U_ITERATIONS = 60
        MyBot.U_DECAY = 0.95
        
        MyBot.E_FACTOR = 0.15
        MyBot.E_ITERATIONS = 90
    ...
    def do_turn(self, ants):
        ...
        # Movement
        # - combat logic
        for ant_loc in sorted(self.soldiers):
            if ant_loc.adj:
                scent_field[ant_loc] = (sum(scent_field[a] 
                                           for a in ant_loc.adj) / 
                                        len(ant_loc.adj))
                moves = [move for move in ant_loc.adj
                                  if (move.unoccupied_next)] + [ant_loc]
                moves.sort(reverse=True,
                       key=lambda a: (scent_field[a], a))           
                neutral_moves = []
                for target in moves:
                    if safety[target] == SAFE:
                        if target != ant_loc:
                        ...
                        break
                    elif safety[target] == NEUTRAL:
                        if target.hilldist <= 8:
                            if target != ant_loc:
                                direction = ant_loc.direction(target)[0]
                                ants.issue_order((ant_loc, direction))
                                target.unoccupied_next = False
                            ...
                            neutral_moves.append(target)
                else: # no currently possible move is SAFE / hillsac
                    if neutral_moves:
                        target = neutral_moves[0]
                        if target != ant_loc:
                            direction = ant_loc.direction(target)[0]
                            ants.issue_order((ant_loc, direction))
                            target.unoccupied_next = False
                        ...
                        self.normal_ants.add(ant_loc)
        
        # - regular movement
        for ant_loc in sorted(self.normal_ants):
        ...
        if Settings.VISUALIZE and 100  <= ants.cur_turn < 130:
            intensities = [[] for i in xrange(256)]
            for cell in ants.explored:
                txts = [ 
                         "u_scent: {0}".format(self.u_scent_field[cell]),
                         "e_scent: {0}".format(5.0*self.e_scent_field[cell]),
                         "a_scent: {0}".format(self.a_scent_field[cell]),
                         "scent: {0}".format(scent_field[cell]),
                         ##"weakness: {0}".format(self.weakness_field[0][cell]),
                         ##"best_enemy: {0}".format(self.lowest_enemy_weakness[cell]),
                         ##"safety_field: {0}".format(safety[cell]),
                        ]
                txt = "; ".join(txts)
                ...
                ##intensity = max(0,min(255, int((safety[cell]+1)/3*256)))
    ...
    def diffuse_food(self, ants):
        ...
        for ant in self.normal_ants:
            clamp_field[ant] = -1.0
        clamps.extend(self.normal_ants)
    ...
    def diffuse_explore(self, ants):
        ...
        # clamped source setup
        clamp_index = unexplored.copy()
        for e_ant in ants.enemy_ants():
            clamp_index[e_ant] = True
        clamp_field = 3.0 * clamp_index
    ...
    def diffuse_attack(self, ants):
        ...
        if my_hills:
            for ant in enemies:
                ...
                if ant.hilldist < INF:
                    clamp_field[ant] = threat_value[ant.hilldist]
                    clamps.append(ant)
                else:
