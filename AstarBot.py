...
class MyBot:
    ...
    def do_turn(self, ants):
        ...
        foodpaths = sorted((a.manhattan(f),f,a) for f,a in 
                        itertools.product(ants.food, ants.my_ants()))
        ##t_begin = time.time()
        num_astar = 0
        i = -1
        for i, (d_af, food_loc, ant_loc) in enumerate(foodpaths):
            if food_loc not in targets and ant_loc not in targets.values():
                if d_af < 9 and num_astar < 7:
                    self.move_astar(ants, ant_loc, food_loc)
                    num_astar += 1
                else:
                    break
        ##if num_astar > 0:
            ##t_foodsearch = 1000*(time.time()-t_begin)/num_astar
            ##log.debug("%s ms each in A* food gathering with %s searches", t_foodsearch, num_astar)
        for (d_af, food_loc, ant_loc) in foodpaths[i:]:
            if food_loc not in targets and ant_loc not in targets.values():
