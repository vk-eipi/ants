...
class Ants():
    ...
    def setup(self, data):
        ...
        self.attack_stamp = location.gen_stamp(self.attackradius2)
    ...
    def run(bot):
        ...
        map_data = ''
        if os.name == 'posix':
            bot.timer_cutoff = True
            signal.signal(signal.SIGALRM, timeout_handler)
        else:
            bot.timer_cutoff = False
        while(True):
            try:
                ...
                # don't raise error or return so that bot attempts to stay 
                # assuming error occurs during turn...
                traceback.print_exc(file=sys.stderr)
                sys.stderr.flush()
                log.error("There was an ERROR!")
                print 'error' # would crash only in strict
                ants.finish_turn()
                map_data = ''
